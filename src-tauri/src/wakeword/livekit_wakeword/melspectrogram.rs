...
use super::{build_session_from_memory, WakeWordError};

const MODEL_BYTES: &[u8] = include_bytes!("onnx/melspectrogram.onnx");

/// Extracts mel-scaled spectrogram features from raw audio using a pre-trained
/// ONNX model.
///
/// Model input:  f32 tensor of shape (1, num_samples) — mono PCM normalized to [-1.0, 1.0]
/// Model output: f32 tensor of shape (1, 1, time_frames, mel_bins) — e.g. (1, 1, 97, 32)
...
impl MelspectrogramModel {
    ...
    /// Run the model on normalized f32 audio and return mel features of shape
