...
//! Upstream's optional input resampling is removed: audio always arrives at
//! 16 kHz from Handy's recorder.
...
use super::melspectrogram::MelspectrogramModel;
use super::{
...
/// Wake word classifier models are loaded from disk at runtime.
///
/// Pass ~2 seconds of 16 kHz i16 PCM to [`predict`](Self::predict) and receive
...
impl WakeWordModel {
    /// Load the feature models plus every classifier in `models`.
    ...
    /// Load a wake word classifier ONNX model from disk. If `model_name` is
    /// `None`, the file stem is used as the classifier name.
    pub fn load_model(
        &mut self,
    ...
    /// Score an audio chunk. Pass ~2 seconds of 16 kHz i16 PCM. Chunks that
    /// yield fewer than [`MIN_EMBEDDINGS`] embeddings score zero.
    pub fn predict(&mut self, audio_chunk: &[i16]) -> Result<HashMap<String, f32>, WakeWordError> {
        if self.classifiers.is_empty() {
            return Ok(HashMap::new());
        }

        let samples_f32: Vec<f32> = audio_chunk.iter().map(|&x| x as f32 / 32768.0).collect();
